    ...
    @isset($server)
        <script>
            ...
            verifyButton.addEventListener('click', function () {
                ...
                axios.post('{{ route('minecraft.admin.servers.verify-azlink', $server) }}', formData)
                    .then(function (json) {
    ...
    <div class="form-group col-md-6">
        ...
        <select class="custom-select @error('type') is-invalid @enderror" id="typeSelect" name="type" required data-toggle-select="server-type">
            @foreach($types as $type)
                <option value="{{ $type }}" @if($type === old('type', $server->type ?? '')) selected @endif>{{ game()->trans($type) }}</option>
            @endforeach
        </select>
...
<div data-server-type="mc-ping" class="form-group d-none">
    <div class="alert alert-info" role="alert">
        <i class="fas fa-info-circle"></i> {{ trans('minecraft::game.servers.ping-no-commands') }}
    </div>
</div>
...
<div data-server-type="source-query source-rcon" class="d-none">
    <div class="form-row">
        <div class="form-group col-md-4">
            <label for="querySourcePortInput">{{ trans('minecraft::game.servers.fields.query-port') }}</label>
            ...
            <small id="queryPortInfo" class="form-text">{{ trans('minecraft::game.servers.query-port-info') }}</small>
        </div>
    </div>
</div>
...
<div data-server-type="source-query" class="form-group d-none">
    <div class="alert alert-info" role="alert">
        <i class="fas fa-info-circle"></i> {{ trans('minecraft::game.servers.query-no-commands') }}
    </div>
</div>
...
<div data-server-type="mc-rcon source-rcon rust-rcon fivem-rcon" class="d-none">
    <div class="form-row">
        <div class="form-group col-md-8">
            <label for="rconPasswordInput">{{ trans('minecraft::game.servers.fields.rcon-password') }}</label>
        ...
        <div class="form-group col-md-4">
            <label for="rconPortInput">{{ trans('minecraft::game.servers.fields.rcon-port') }}</label>
            ...
            <small id="rconPortInfo" class="form-text">{{ trans('minecraft::game.servers.query-port-info') }}</small>
        </div>
    </div>
</div>
...
<div data-server-type="mc-azlink" class="d-none">
    <div class="form-group custom-control custom-switch">
        ...
        <label class="custom-control-label" for="hasPingSwitch">{{ trans('minecraft::game.servers.azlink.enable-ping') }}</label>

        <small class="form-text" id="pingInfo">{{ trans('minecraft::game.servers.azlink.ping-info') }}</small>
    </div>
    ...
    <div id="hasPingGroup" class="@if(isset($server) && ($server->data['azlink-ping'] ?? false)) show @else collapse @endisset">
        <div class="form-group custom-control custom-switch">
            ...
            <label class="custom-control-label" for="customPortSwitch">{{ trans('minecraft::game.servers.azlink.custom-port') }}</label>
        </div>

        <div id="customPortGroup" class="@isset($server->data['azlink-port']) show @else collapse @endisset">
            <div class="card card-body mb-3">
                <div class="form-group">
                    <label for="azlinkPortInput">{{ trans('minecraft::game.servers.fields.azlink-port') }}</label>
                ...
                <div class="alert alert-info mb-0" role="alert">
                    ...
                    {{ trans('minecraft::game.servers.azlink.port-info') }}
    ...
    @isset($server)
        <div class="form-group">
            @if($server->isOnline())
                <div class="alert alert-info" role="alert">
                    <i class="fas fa-info-circle"></i>
                    {{ trans('minecraft::game.servers.azlink.link-info') }}
            ...
            @else
                <div class="alert alert-primary" role="alert">
                    {{ trans('minecraft::game.servers.azlink.link') }}
                    <ol class="mb-0">
                        <li>@lang('admin.servers.azlink.link-1')</li>
                        <li>{{ trans('minecraft::game.servers.azlink.link-2') }}</li>
                        <li>
                            {{ trans('minecraft::game.servers.azlink.link-3') }}
                            <code id="linkCommand" class="cursor-copy" title="{{ trans('messages.actions.copy') }}" data-copied="{{ trans('messages.copied') }}" data-toggle="tooltip" data-clipboard-target="#linkCommand">{{ $server->getLinkCommand() }}</code>
                            .
                        </li>
                    </ol>
